const timer = () => {
  ...
  const updateTimer = () => {
    const date = new Date();
    const dateDeadLine = new Date('28 april 2022').getTime(); // до какой даты
    ...
    dayBlock.nextElementSibling.textContent = numWord(days, ['день', 'дня', 'дней']);
